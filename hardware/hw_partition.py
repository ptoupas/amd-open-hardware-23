...
#         #  create processes
#         q = mp.Queue(5000)
#         thread_in = mp.Process(target = run_fifo_in, args=(q,))
#         thread_out = mp.Process(target = run_fifo_out, args=(q,))
...
#                 if (self.dma[dma_from_idx[i]].recvchannel._first_transfer or \
#                         self.dma[dma_from_idx[i]].recvchannel.idle) and cntr_in[i] < cntr_max[i]:
                    cntr_in[i] += 1
...
#                 if (self.dma[dma_to_idx[i]].sendchannel._first_transfer or \
#                         self.dma[dma_to_idx[i]].sendchannel.idle) and cntr_out[i] < cntr_max[i] \
#                         and cntr_out[i] < cntr_in[i]:
                    cntr_out[i] += 1
...
#     fifo_0 = mp.Process(target=partition.start_fifo, args=(0,))
#     fifo_1 = mp.Process(target=partition.start_fifo, args=(1,))
    
    # assign the input data
    data = np.moveaxis(input_data, 0, -1).flatten() * (2**partition.input_bp[0])
    partition.input_buffers[0][:] = data.astype(np.int16)
    
#     print(partition.input_buffers[0][:])
...
#     partition.dma[0].recvchannel.transfer(
#             partition.fifo_buffers[0][0])  
#     partition.dma[1].recvchannel.transfer(
#             partition.fifo_buffers[1][0])      
...
#     fifo_1.join()
    ...
    # print(f"fpgaconvnet executed! ({pred_time:.2f})")

    # reset the FIFO
    partition.reset_fifo_dma()

    # get the outputs
#     print("fifo",  [ hex(x & 0xFFFF) for x in partition.fifo_buffers[0][0][:128]])
#     print("output", [ hex(x & 0xFFFF) for x in partition.output_buffers[2][:64]])
