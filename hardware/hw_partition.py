...
class Partition:
    ...
    def reload_weights(self, index: int, weights_filepath: str):
        ...
        with open(weights_filepath, "r") as f:
            weights = np.array([int(x, base=16) & 0xFFFF \
        ...
        # flush the weights buffer
        weight_buffers.flush()
        ...
        # wait a bit for axi commands to affect hardware
        time.sleep(0.005)
        ...
        # wait for weight updates to happen
        time.sleep(0.005)
...
def run_fpgaconvnet(partition, input_data):
    ...
    # get the outputs
    output_0 = np.moveaxis(np.reshape(partition.output_buffers[2].copy(), 
        partition.output_shape[2]), -1, 0).astype(np.float32) / float(2**partition.output_bp[2])
    output_1 = np.moveaxis(np.reshape(partition.output_buffers[3].copy(),
        partition.output_shape[3]), -1, 0).astype(np.float32) / float(2**partition.output_bp[3])
    output_2 = np.moveaxis(np.reshape(partition.output_buffers[4].copy(),
        partition.output_shape[4]), -1, 0).astype(np.float32) / float(2**partition.output_bp[4])
#     output_2 = np.moveaxis(np.zeros(partition.output_shape[4]), -1, 0).astype(np.float32) / float(2**partition.output_bp[4])
